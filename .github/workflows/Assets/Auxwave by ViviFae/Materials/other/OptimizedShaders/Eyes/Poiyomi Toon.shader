Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/ca1e530b91edc2b4e84a9970deed8c7e"
{
	Properties
	{
		...
		[HideInInspector] s_start_MainHueShift_Eyes ("Hue Shift--{reference_property:_MainHueShiftToggle,persistent_expand:true,default_expand:true}", Float) = 1
		...
		_MainHueShift_Eyes ("Hue Shift", Range(0, 1)) = 0
		...
		[HideInInspector] s_end_MainHueShift_Eyes ("Name Motion", Float) = 0
		...
		_DecalBlendAlpha_Eyes ("Alpha", Range(0, 1)) = 1
 ...
 #define OPTIMIZER_ENABLED 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.finalLighting = max(POI_BRDF_PBS(1, 0, 0, 0, poiMesh.normals[1], poiCam.viewDir, light, indirectLight).xyz, (0.07 /*_LightingMinLightBrightness*/));
				#endif
			...
			struct PoiDecal
			{
				...
				void Apply(inout float alphaOverride, inout float decalAlpha, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
				{
					...
					float decalAlphaMixed = decalColor.a * saturate(m_DecalBlendAlpha_Eyes + audioLinkDecalAlpha);
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				Decal0.m_DecalColor = float4(0.005181514,0.05612848,1,1);
				...
				Decal0.m_DecalBlendAlpha_Eyes = _DecalBlendAlpha_Eyes;
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				applyALEmmissionStrength(poiMods, emissionStrength0, float4(1,2,0,0), (0.0 /*_EmissionAL0StrengthBand*/), float4(1,2,0,0), (0.0 /*_EmissionAL0MultipliersBand*/), _EmissionAL0Enabled);
				...
				{
					emissionColor0 = hueShift(emissionColor0, frac(_EmissionHueShift_Eyes + (0.0 /*_EmissionHueShiftSpeed*/) * _Time.x), (0.0 /*_EmissionHueShiftColorSpace*/), (1.0 /*_EmissionHueSelectOrShift*/));
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				{
					poiLight.directColor = max(poiLight.directColor, (0.07 /*_LightingMinLightBrightness*/));
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor * min(10000, ((0.07 /*_LightingMinLightBrightness*/) * rcp(calculateluminance(poiLight.directColor)))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor * min(10000, ((0.07 /*_LightingMinLightBrightness*/) * rcp(calculateluminance(poiLight.indirectColor)))));
				}
