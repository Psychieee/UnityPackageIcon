Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/471994c1e7fe30945b7f8621fde41882"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (0.0)
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)) + (dot(pos, float4(0,20,0,0).xyz) * 25.0) + _Time.x * 3.0).r;
						#endif
					}
					else
					{
						emissionStrength0 *= calculateScrollingEmission(float4(0,20,0,0).xyz, 3.0, 25.0, 20.0, 0.0, pos);
					}
				}
				if (0.0)
				{
			...
			float3 applyEmission__1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				float emissionStrength0__1 = 0.0;
				...
				float glowInTheDarkMultiplier0__1 = calculateGlowInTheDark__1(0.0, 1.0, 1.0, 0.15, 1.0, 0.0, poiLight);
				#ifdef POI_GRABPASS
				...
				if (1.0)
				{
					emissionStrength0__1 *= calculateBlinkingEmission__1(0.15, 1.0, 1.0, 10.0);
				}
				if(0.0)
				{
