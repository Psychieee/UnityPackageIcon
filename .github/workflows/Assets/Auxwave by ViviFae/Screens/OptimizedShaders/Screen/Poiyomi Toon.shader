Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/1aaa92a9d386ad44b8a57f015afed77c"
{
	Properties
	{
		...
		[sRGBWarning(true)]_MainTex_Screen ("Texture--{reference_properties:[_MainTexPan, _MainTexUV, _MainPixelMode, _MainTexStochastic, _MainIgnoreTexAlpha]}", 2D) = "white" { }
		...
		_Saturation_Screen ("Saturation", Range(-1, 10)) = 0
		_MainBrightness_Screen ("Brightness", Range(-1, 2)) = 0
		_MainGamma ("Gamma", Range(0.01, 5)) = 1
		[HideInInspector] s_start_MainHueShift_Screen ("Hue Shift--{reference_property:_MainHueShiftToggle,persistent_expand:true,default_expand:true}", Float) = 1
		...
		_MainHueShift_Screen ("Hue Shift", Range(0, 1)) = 0
		...
		[HideInInspector] s_end_MainHueShift_Screen ("Name Motion", Float) = 0
		...
		_DecalEmissionStrength1_Screen ("Emission Strength", Range(0, 20)) = 0
		...
		_DecalBlendAlpha1_Screen ("Alpha", Range(0, 1)) = 1
		...
		_Decal1VideoEmissionStrength_Screen ("Emission", Range(0, 20)) = 0
		...
		_DecalBlendAlpha2_Screen ("Alpha", Range(0, 1)) = 1
		...
		[VectorLabel(Xmin, Ymin, Xmax, Ymax)]_AudioLinkDecal2Scale_Screen ("Scale Mod", Vector) = (0, 0, 0, 0)
		[Space(7)]
		...
		[VectorLabel(Min, Max)]_AudioLinkDecal2Emission_Screen ("Emission Mod", Vector) = (0, 0, 0, 0)
		[ToggleUI]_AudioLinkDecalCC2_Screen ("CC Strip", Float) = 0
		[Space(7)]
		...
		_FlipbookReplace_Screen ("Replace", Range(0, 1)) = 1
		_FlipbookEmissionStrength ("Emission Strength", Range(0, 20)) = 0
		_FlipbookFPS_Screen ("FPS", Float) = 30.0
 ...
 #define OPTIMIZER_ENABLED 
			...
			UNITY_DECLARE_TEX2D(_MainTex_Screen);
			...
			float2 _MainTexPan;
			float _MainTexUV;
			float4 _MainTex_Screen_TexelSize;
			...
			float _MainHueShift_Screen;
			float _MainHueShiftSpeed;
			float _Saturation_Screen;
			float _MainBrightness_Screen;
			float _MainGamma;
			...
			float4 _AudioLinkDecal2Scale_Screen;
			...
			float _Decal1VideoEmissionStrength_Screen;
			...
			float _DecalEmissionStrength1_Screen;
			float _DecalBlendAlpha1_Screen;
			...
			float _DecalBlendAlpha2_Screen;
			...
			{
				float3 normal = UnpackScaleNormal(POI2D_SAMPLER_PAN(normalTexture, _MainTex_Screen, poiUV(poiMesh.uv[normalUV], normal_ST), normalPan), normalIntensity);
				return normalize(
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				#if defined(PROP_DECALMASK)
				float4 decalMask = POI2D_SAMPLER_PAN(_DecalMask, _MainTex_Screen, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				float4 decalMask = 1;
				#endif
				...
				Decal1.m_DecalEmissionStrength = _DecalEmissionStrength1_Screen;
				Decal1.m_DecalBlendAlpha = _DecalBlendAlpha1_Screen;
				...
				else
				{
					...
					if (1.0)
					{
						if (udonVideoTexAvailable)
						{
							Decal1.m_DecalEmissionStrength += _Decal1VideoEmissionStrength_Screen;
							if (1.0)
							{
								Decal1.SampleDecal(_DecalTexture1, poiMods, poiLight, poiMesh, poiCam);
				...
				Decal2.m_DecalScale = float4(0.75,0.75,1,0);
				...
				Decal2.m_DecalBlendAlpha = _DecalBlendAlpha2_Screen;
				...
				Decal2.m_AudioLinkDecalScale = _AudioLinkDecal2Scale_Screen;
				...
				Decal2.m_AudioLinkDecalEmission = _AudioLinkDecal2Emission_Screen;
				Decal2.m_DecalRotationCTALBand = 0.0;
				Decal2.m_DecalRotationCTALSpeed = 0.0;
				Decal2.m_DecalRotationCTALType = 0.0;
				Decal2.m_AudioLinkDecalColorChord = _AudioLinkDecalCC2_Screen;
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float flipBookMask = POI2D_SAMPLER_PAN(_FlipbookMask, _MainTex_Screen, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0))[0.0];
				#else
				...
				if (!0.0)
				{
					if (_FlipbookFPS_Screen != 0)
					{
						currentFrame = ((_Time.y / (1 / _FlipbookFPS_Screen)) + 0.0) % totalFrames;
						if (0.0)
						{
							currentFrame += 0.0;
						}
				...
				float finalFlipbookAlpha = saturate(flipBookPixel.a * float4(1,1,1,1).a * _FlipbookReplace_Screen * flipBookMask * flipbookAlpha);
				if (0.0 == 2)
				{
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!0.0)
				{
					emissionTex = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex_Screen, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				}
				else
				{
					emissionTex = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex_Screen, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * 5.0);
				}
				...
				if (0.0)
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex_Screen, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)) + (dot(pos, float4(0,-10,0,0).xyz) * 20.0) + _Time.x * 10.0).r;
						#endif
					}
					else
					{
				...
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
				float emissionMask0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask, _MainTex_Screen, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)) + _Time.x * float4(0,0,0,0))[0.0];
				#else
			...
			void applyVideoEffectsMainTex(inout float4 mainTexture, in PoiMesh poiMesh)
			{
				...
				if (0.0)
				{
					...
					uvs = poiUV(uvs, _MainTex_Screen_ST) + _Time.x * float4(0,0,0,0);
					mainTexture = _MainTex_Screen.SampleGrad(sampler_MainTex_Screen, uvs, ddx(originalUVs), ddy(originalUVs));
				}
			}
			void applyVideoEffects(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				#if defined(PROP_VIDEOMASKTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float screenMask = POI2D_SAMPLER_PAN(_VideoMaskTexture, _MainTex_Screen, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0))[0.0];
				#else
				float screenMask = 1;
				#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[0.0].xy, _MainTex_Screen_ST);
				if (0.0)
				{
					mainUV = sharpSample(_MainTex_Screen_TexelSize, mainUV);
				}
				float4 mainTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_MainTex_Screen, _MainTex_Screen, mainUV, float4(0,0,0,0), 0.0);
				...
				#endif
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex_Screen, poiUV(poiMesh.uv[0.0].xy, float4(1,1,0,0)), float4(0,0,0,0), 0.0), 1.0);
				#else
				...
				#endif
				#if defined(PROP_LIGHTINGAOMAPS)
				float4 AOMaps = POI2D_SAMPLER_PAN(_LightingAOMaps, _MainTex_Screen, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				...
				#if defined(PROP_LIGHTINGDETAILSHADOWMAPS)
				float4 DetailShadows = POI2D_SAMPLER_PAN(_LightingDetailShadowMaps, _MainTex_Screen, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				#ifndef POI_PASS_ADD
				...
				#if defined(PROP_LIGHTINGSHADOWMASKS)
				float4 ShadowMasks = POI2D_SAMPLER_PAN(_LightingShadowMasks, _MainTex_Screen, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				...
				#ifdef COLOR_GRADING_HDR
				#if defined(PROP_MAINCOLORADJUSTTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 hueShiftAlpha = POI2D_SAMPLER_PAN(_MainColorAdjustTexture, _MainTex_Screen, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				...
				if (1.0 == 1)
				{
					float shift = _MainHueShift_Screen;
					#ifdef POI_AUDIOLINK
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), - (_Saturation_Screen) * hueShiftAlpha.b);
				poiFragData.baseColor = saturate(lerp(poiFragData.baseColor, poiFragData.baseColor * (_MainBrightness_Screen + 1), hueShiftAlpha.g));
				#endif
				if (2.0)
				{
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
					float alphaMask = POI2D_SAMPLER_PAN(_AlphaMask, _MainTex_Screen, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0).xy).r;
					#else
					float alphaMask = 1;
					#endif
