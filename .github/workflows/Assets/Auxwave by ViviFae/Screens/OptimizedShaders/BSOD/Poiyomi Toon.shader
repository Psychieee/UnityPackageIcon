Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/42d56f1ab3046014bb9a339f5545bb1a"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				#if defined(PROP_VIDEOPIXELTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float3 pixels = tex2D(_VideoPixelTexture, poiUV(poiMesh.uv[0.0], float4(0.5,0.5,0,0)) * float4(1280,720,0,0));
				#else
				float3 pixels = 1;
				#endif
				float2 uvs = poiMesh.uv[0.0];
				...
				#if defined(PROP_VIDEOMASKTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float screenMask = POI2D_SAMPLER_PAN(_VideoMaskTexture, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0))[0.0];
				#else
				float screenMask = 1;
				#endif
