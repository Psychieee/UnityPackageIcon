Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/dd405c93bbce46346b4170a9aafc8f8a"
{
	Properties
	{
		...
		_MainBrightness_ScreenAlt ("Brightness", Range(-1, 2)) = 0
		...
		[HideInInspector][ThryWideEnum(UV0, 0, UV1, 1, UV2, 2, UV3, 3, Panosphere, 4, World Pos, 5, Local Pos, 8, Polar UV, 6, Distorted UV, 7, Matcap, 9)] _FlipbookTexArrayUV ("UV", Int) = 0
		...
		[HideInInspector][ThryWideEnum(UV0, 0, UV1, 1, UV2, 2, UV3, 3, Panosphere, 4, World Pos, 5, Local Pos, 8, Polar UV, 6, Distorted UV, 7, Matcap, 9)] _FlipbookMaskUV ("UV", Int) = 0
		...
		[HideInInspector][ThryWideEnum(UV0, 0, UV1, 1, UV2, 2, UV3, 3, Panosphere, 4, World Pos, 5, Local Pos, 8, Polar UV, 6, Distorted UV, 7, Matcap, 9)]_VideoMaskTextureUV ("UV", Int) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDecals(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, inout PoiMods poiMods, in PoiLight poiLight)
			{
				...
				#if defined(PROP_DECALMASK)
				float4 decalMask = POI2D_SAMPLER_PAN(_DecalMask, _MainTex_ScreenAlt, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				float4 decalMask = 1;
				#endif
				...
				Decal0.m_DecalChannelSeparation = 0.82;
				Decal0.m_DecalChannelSeparationPremultiply = 0.0;
				Decal0.m_DecalChannelSeparationHue = 0.0;
				Decal0.m_DecalChannelSeparationVertical = -0.56;
				Decal0.m_DecalChannelSeparationAngleStrength = -2.52;
				...
				Decal0.m_AudioLinkDecalEmissionBand = 4.0;
				...
				{
					udonVideoAspectRatio = Decal1.GetVideoAspectRatio(_Udon_VideoTex_TexelSize.zw, 1.0, 1.0);
					if (1.0)
					{
						if (udonVideoTexAvailable)
						{
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float flipBookMask = POI2D_SAMPLER_PAN(_FlipbookMask, _MainTex_ScreenAlt, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0))[0.0];
				#else
				...
				if (!0.0)
				{
					if (30.0 != 0)
					{
						currentFrame = ((_Time.y / (1 / 30.0)) + 0.0) % totalFrames;
						if (0.0)
						{
							currentFrame += 0.0;
						}
				...
				#endif
				float2 finalUV = TRANSFORM_TEX(newUV, _FlipbookTexArray) + _Time.x * float4(0,0,0,0);
				...
				if (0.0)
				{
					flipBookPixel.a = poiMax(flipBookPixel.rgb) * edgeAlpha;
				}
				else
				{
				...
				float finalFlipbookAlpha = saturate(flipBookPixel.a * float4(1,1,1,1).a * _FlipbookReplace_ScreenAlt * flipBookMask * flipbookAlpha);
				if (0.0 == 2)
				{
				...
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor.rgb, customBlend(poiFragData.baseColor.rgb, flipBookPixel.rgb, 6.0), finalFlipbookAlpha);
				}
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				applyALEmmissionStrength(poiMods, emissionStrength0, float4(1,0,0,0), 0.0, float4(1,2,0,0), 0.0, 0.0);
				...
				if (!0.0)
				{
					emissionTex = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex_ScreenAlt, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				}
				else
				{
					emissionTex = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex_ScreenAlt, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * 5.0);
				}
				...
				if (0.0)
				{
					...
					{
						#if defined(PROP_EMISSIONSCROLLINGCURVE) || !defined(OPTIMIZER_ENABLED)
						emissionStrength0 *= UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionScrollingCurve, _MainTex_ScreenAlt, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)) + (dot(pos, float4(0,-10,0,0).xyz) * 20.0) + _Time.x * 10.0).r;
						#endif
					}
					else
					{
				...
				#if defined(PROP_EMISSIONMASK) || !defined(OPTIMIZER_ENABLED)
				float emissionMask0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMask, _MainTex_ScreenAlt, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)) + _Time.x * float4(0,0,0,0))[0.0];
				#else
			...
			float calculateCRTPixelBrightness(float2 uv)
			{
				...
				half electronBeamAlpha = frac(_Time.y * 24.0);
			...
			void applyContrastSettings(inout float3 pixel)
			{
				...
				#endif
				pixel = saturate(lerp(half3(0.5, 0.5, 0.5), pixel, 0.0 + 1));
				#if !UNITY_COLORSPACE_GAMMA
			...
			{
				pixel = lerp(pixel.rgb, dot(pixel.rgb, float3(0.3, 0.59, 0.11)), - (0.0));
			}
			...
			{
				videoTexture.rgb = applyBacklight(videoTexture, 1.0 * .01);
				applyVideoSettings(videoTexture.rgb);
				videoTexture.rgb = videoTexture * pixels * 1.0;
			}
			...
			void calculateCRT(inout float4 videoTexture, float3 pixels, float2 uv)
			{
				...
				applyVideoSettings(videoTexture.rgb);
				videoTexture.rgb = videoTexture * pixels * brightness * 1.0;
			}
			...
			void calculateGameboy(inout float4 videoTexture)
			{
				...
				videoTexture.rgb = _VideoGameboyRamp.SampleLevel(sampler_linear_clamp, brightness.xx, 0).rgb;
				#else
			...
			{
				applyVideoSettings(videoTexture.rgb);
				float3 projectorColor = videoTexture * 1.0;
			...
			{
				#if defined(PROP_VIDEOPIXELTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float3 pixels = tex2D(_VideoPixelTexture, poiUV(poiMesh.uv[0.0], float4(0.4,0.4,0,0)) * float4(1280,720,0,0));
				#else
				float3 pixels = 1;
				#endif
				float2 uvs = poiMesh.uv[0.0];
				...
				#if defined(PROP_VIDEOMASKTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float screenMask = POI2D_SAMPLER_PAN(_VideoMaskTexture, _MainTex_ScreenAlt, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0))[0.0];
				#else
				float screenMask = 1;
				#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float2 mainUV = poiUV(poiMesh.uv[0.0].xy, _MainTex_ScreenAlt_ST);
				if (0.0)
				{
					mainUV = sharpSample(_MainTex_ScreenAlt_TexelSize, mainUV);
				}
				float4 mainTexture = POI2D_SAMPLER_PAN_STOCHASTIC(_MainTex_ScreenAlt, _MainTex_ScreenAlt, mainUV, float4(0,0,0,0), 0.0);
				...
				#endif
				#if defined(PROP_BUMPMAP) || !defined(OPTIMIZER_ENABLED)
				poiMesh.tangentSpaceNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN_STOCHASTIC(_BumpMap, _MainTex_ScreenAlt, poiUV(poiMesh.uv[0.0].xy, float4(1,1,0,0)), float4(0,0,0,0), 0.0), 1.0);
				#else
				...
				#endif
				#if defined(PROP_LIGHTINGAOMAPS)
				float4 AOMaps = POI2D_SAMPLER_PAN(_LightingAOMaps, _MainTex_ScreenAlt, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				...
				#if defined(PROP_LIGHTINGDETAILSHADOWMAPS)
				float4 DetailShadows = POI2D_SAMPLER_PAN(_LightingDetailShadowMaps, _MainTex_ScreenAlt, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				#ifndef POI_PASS_ADD
				...
				#if defined(PROP_LIGHTINGSHADOWMASKS)
				float4 ShadowMasks = POI2D_SAMPLER_PAN(_LightingShadowMasks, _MainTex_ScreenAlt, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				...
				poiFragData.baseColor *= poiThemeColor(poiMods, float4(0,0.6998986,1,1).rgb, 0.0);
				poiFragData.alpha = mainTexture.a * float4(0,0.6998986,1,1).a;
				#else
				...
				#ifdef COLOR_GRADING_HDR
				#if defined(PROP_MAINCOLORADJUSTTEXTURE) || !defined(OPTIMIZER_ENABLED)
				float4 hueShiftAlpha = POI2D_SAMPLER_PAN(_MainColorAdjustTexture, _MainTex_ScreenAlt, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0));
				#else
				...
				{
					#if defined(PROP_ALPHAMASK) || !defined(OPTIMIZER_ENABLED)
					float alphaMask = POI2D_SAMPLER_PAN(_AlphaMask, _MainTex_ScreenAlt, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0).xy).r;
					#else
					float alphaMask = 1;
					#endif
